...
<body style="font-family: 'Poppins', sans-serif !important;">
    ...
    WHERE
        ...
        if ($result) {
            ...
            echo "<button class='btn btn-primary printbtn text-end mb-3' onclick='printAttendanceReport()'>Print Attendance Report</button>
    ...
    <script>
        ...
        function printAttendanceReport() {
            ...
            var containerMain = document.querySelector('.container.main').cloneNode(true);

            // Remove the print button from the cloned container
            var printButton = containerMain.querySelector('.printbtn');
            if (printButton) {
                printButton.remove();
            }

            // Extract the table rows from the cloned container
            ...
            // Include the additional information before the table
            printWindow.document.write('<div class="container">');
            printWindow.document.write('<p class="text-center fs-6" style="margin-bottom: 20px ;"><strong>Branch:</strong> ' + <?php echo json_encode($db . '(' . $branch . ')'); ?> + ' | <strong>Year:</strong> ' + <?php echo json_encode($year); ?> + ' | <strong>Semester:</strong> ' + <?php echo json_encode($semester); ?> + ' | <strong>Session Type:</strong> ' + <?php echo json_encode($session_type); ?> + ' | <strong>Division:</strong> ' + <?php echo json_encode($division); ?> + ' | <strong>Date:</strong> ' + <?php echo json_encode($dd); ?> + '</p>');
            printWindow.document.write('<p class="text-center fs-5"><strong>Subject:</strong> ' + <?php echo json_encode($ds . ' (' . $subject_name . ')'); ?> + '</p>');
            ...
            printWindow.document.write('<th>Roll No</th>');
            printWindow.document.write('<th>Enrollment Number</th>');
            printWindow.document.write('<th>Student Name</th>');
            printWindow.document.write('<th>Attendance Status</th>');
            printWindow.document.write('</tr>');
            printWindow.document.write('</thead>');
            ...
            tableRows.forEach(function(row) {
                ...
                var rollNo = cells[0].textContent;
                var enrollmentNo = cells[1].textContent;
                var studentName = cells[2].textContent;
                var attendanceStatus = cells[3].textContent;

                // Write the row data to the new window
                printWindow.document.write('<tr>');
                printWindow.document.write('<td style="text-align:center;">' + rollNo + '</td>');
                printWindow.document.write('<td style="text-align:center;">' + enrollmentNo + '</td>');
                printWindow.document.write('<td style="text-align:center;">' + studentName + '</td>');
                printWindow.document.write('<td style="text-align:center;">' + attendanceStatus + '</td>');
                printWindow.document.write('</tr>');
            });
